    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            jdbcClass.createDbUserTable(
                    "CREATE TABLE USERS(USER_ID INTEGER  UNIQUE, NAME VARCHAR(50) NOT NULL, ADDRESS VARCHAR(255), PRIMARY KEY(USER_ID));"
                            + "CREATE TABLE ACCOUNTS(ACCOUNT_ID INTEGER  UNIQUE, USER_ID INTEGER NOT NULL, BALANCE INTEGER CHECK(BALANCE >= 0) CHECK(BALANCE <= 1000000000), CURRENCY VARCHAR(15) NOT NULL, PRIMARY KEY(ACCOUNT_ID), FOREIGN KEY (USER_ID) REFERENCES USERS(USER_ID)); "
                            + "CREATE TABLE TRANSACTIONS(TRANSACTION_ID INTEGER  NOT NULL, ACCOUNT_ID INTEGER NOT NULL, AMOUNT INTEGER NOT NULL, PRIMARY KEY(TRANSACTION_ID), FOREIGN KEY (ACCOUNT_ID) REFERENCES ACCOUNTS(ACCOUNT_ID))");
...
//            System.out.println("Транзакции сделаны");
          ...
          //  jdbcClass.transaction2(1, 3000,"US");
            jdbcClass.transaction2(1, 2000,"US");
            System.out.println("тут хер пойми что делается");
